...
We first show that amino acid k-mers (k~aa~-mers) accurately estimate some features of microbial pangenomes by comparing amino acid profiles of proteomes (translated coding domain sequences) against the proteomes themselves (**Figure @fig:overview A**).
...
We repeated this process a second time but mapped in amino acid space, which improves sensitivity over nucleotide mapping [@doi:10.1093/bioinformatics/btx021].
The fraction of reads that mapped increased by an average of 1.7% (Welch Two Sample Paired t-test, estimate = 1.9, p < 0.001), accounting for 94.6% of total reads and indicating that a substantial fraction of distinct sequences in the k~aa~-mer metapangenome represent diverged genomic sequences that retains amino acid similarity.
